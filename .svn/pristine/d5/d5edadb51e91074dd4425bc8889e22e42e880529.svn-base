...
public class SurveyAsdBdi02WriterMapper implements CellWriterMapper<SurveyAsdBdi02VO>{
	@Override
	public void mapHeader(Row row, SurveyAsdBdi02VO vo) {
		Cell cell = null;
		int i = 0;
		...
		if(rowNum==0) {
			...
			cell.setCellValue("아동자가평가 (BDI 성인용)");
		...
		if(rowNum==1) {
			...
			cell.setCellValue("체중조절");cell = row.createCell(i++);
	...
	@Override
	public void mapRow(Row row, SurveyAsdBdi02VO vo) {
		Cell cell = null;
		int i = 0;
		cell = row.createCell(i++);
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getBdiExecDate());

		cell = row.createCell(i++);
		cell.setCellValue(vo.getScore());cell = row.createCell(i++);
	...
	public void cellMerge(Sheet sheet) {
		...
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 7, 29));
	}
