...
public class SurveyAsdSsis1WriterMapper implements CellWriterMapper<SurveyAsdSsis1VO>{
	@Override
	public void mapHeader(Row row, SurveyAsdSsis1VO vo) {
		Cell cell = null;
		int i = 0;
		...
		if(rowNum==1) {
			...
			cell.setCellValue("총점");cell = row.createCell(i++);
			...
			cell.setCellValue("사회적기술 총점");cell = row.createCell(i++);
			...
			cell.setCellValue("79");cell = row.createCell(88);
			cell.setCellValue("문제행동 총점");
			
		}
	}
	
	@Override
	public void mapRow(Row row, SurveyAsdSsis1VO vo) {
		Cell cell = null;
		int i = 0;
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getSsisExecDate());

		cell = row.createCell(i++);
		
		cell.setCellValue(vo.getScore());cell = row.createCell(i++); //총점
		...
		cell.setCellValue(vo.getA80());cell = row.createCell(i++); //사회적기술 총점
		...
		cell.setCellValue(vo.getA81()); //문제행동 총점
	...
	public void cellMerge(Sheet sheet) {
		...
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 7, 88));
