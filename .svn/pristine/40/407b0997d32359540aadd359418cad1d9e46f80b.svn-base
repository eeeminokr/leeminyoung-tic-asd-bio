...
public class SurveyAsdTrauaWriterMapper implements CellWriterMapper<SurveyAsdTrauaVO>{
	@Override
	public void mapHeader(Row row, SurveyAsdTrauaVO vo) {
		Cell cell = null;
		int i = 0;
		...
		if(rowNum==1) {
			...
			cell.setCellValue("가장 큰 영향을 준 항목");cell = row.createCell(i++);
			cell.setCellValue("공포/무력감");cell = row.createCell(36);
			cell.setCellValue("꿈을 꾸는 느낌");
			
		}
	}
	
	@Override
	public void mapRow(Row row, SurveyAsdTrauaVO vo) {
		Cell cell = null;
		int i = 0;
		cell = row.createCell(i++);
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getTrauaExecDate());
	...
	public void cellMerge(Sheet sheet) {
		...
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 7, 36));
