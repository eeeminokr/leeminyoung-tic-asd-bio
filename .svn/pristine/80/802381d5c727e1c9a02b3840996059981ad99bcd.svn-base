...
public class SurveyAsdRcmasWriterMapper implements CellWriterMapper<SurveyAsdRcmasVO>{
	@Override
	public void mapHeader(Row row, SurveyAsdRcmasVO vo) {
		Cell cell = null;
		int i = 0;
	...
	@Override
	public void mapRow(Row row, SurveyAsdRcmasVO vo) {
		Cell cell = null;
		int i = 0;
		cell = row.createCell(i++);
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getRcmasExecDate());
	...
	public void cellMerge(Sheet sheet) {
		...
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 7, 44));
