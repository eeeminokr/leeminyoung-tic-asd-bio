...
public class NeuropsyTmtCellReaderMapper implements CellReaderMapper<NeuropsyTmtVO> {
	...
	public NeuropsyTmtVO mapCell(Sheet sheet, int col, int row)
		...
		try{
			...
			neuropsyTmtVO.setPerformCntNm(POIUtil.getStringCellValue(sheet.getRow(row).getCell(1)).trim().replace("'",""));
			neuropsyTmtVO.setTmtExecDate(execDate.replace("-", ""));
			neuropsyTmtVO.setTmtArt(POIUtil.getStringCellValue(sheet.getRow(row).getCell(3)).trim().replace("'",""));
			neuropsyTmtVO.setTmtAError(POIUtil.getStringCellValue(sheet.getRow(row).getCell(4)).trim().replace("'",""));
			neuropsyTmtVO.setTmtBrt(POIUtil.getStringCellValue(sheet.getRow(row).getCell(5)).trim().replace("'",""));
			neuropsyTmtVO.setTmtBError(POIUtil.getStringCellValue(sheet.getRow(row).getCell(6)).trim().replace("'",""));
			neuropsyTmtVO.setRemarks(POIUtil.getStringCellValue(sheet.getRow(row).getCell(7)).trim().replace("'",""));
