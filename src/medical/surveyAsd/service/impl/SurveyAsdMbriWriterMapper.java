...
public class SurveyAsdMbriWriterMapper implements CellWriterMapper<SurveyAsdMbriVO>{
	@Override
	public void mapHeader(Row row, SurveyAsdMbriVO vo) {
		Cell cell = null;
		int i = 0;
		...
		if(rowNum==1) {
			...
			cell = row.createCell(i++);
			cell.setCellValue("애정적\n태도");
			cell = row.createCell(i++);
			cell.setCellValue("거부적\n태도");
			cell = row.createCell(i++);
			cell.setCellValue("자율적\n태도");
			cell = row.createCell(i++);
			cell.setCellValue("통제적\n태도");
		}
	}
	
	@Override
	public void mapRow(Row row, SurveyAsdMbriVO vo) {
		Cell cell = null;
		int i = 0;
		cell = row.createCell(i++);
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getMbriExecDate());
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getAffectionateAttitude());
		
		cell = row.createCell(i++);
		cell.setCellValue(vo.getDenyAttitude());
		
		cell = row.createCell(i++);
		cell.setCellValue(vo.getAutonomousAttitude());
		
		cell = row.createCell(i++);
		cell.setCellValue(vo.getControlledAttitude());
	}
	...
	public void cellMerge(Sheet sheet) {
		...
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 7, 59));
