...
public class SurveyAsdBpi01WriterMapper implements CellWriterMapper<SurveyAsdBpi01VO>{
	@Override
	public void mapHeader(Row row, SurveyAsdBpi01VO vo) {
		Cell cell = null;
		int i = 0;
		...
		if(rowNum==1) {
			...
			cell.setCellValue("12");
			cell = row.createCell(62);
			cell.setCellValue("자기파괴적");
		}
	}
	
	@Override
	public void mapRow(Row row, SurveyAsdBpi01VO vo) {
		Cell cell = null;
		int i = 0;
		cell = row.createCell(i++);
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getBpi01ExecDate());

		cell = row.createCell(i++);
		cell.setCellValue(vo.getTotalScore());cell = row.createCell(i++);
		...
		cell.setCellValue(vo.getSelfInjury());cell = row.createCell(i++);
		...
		cell.setCellValue(vo.getStereotypy());cell = row.createCell(i++);
		...
		cell.setCellValue(vo.getSelfDestructive());cell = row.createCell(i++);		
	}
	...
	public void cellMerge(Sheet sheet) {
		...
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 7, 62));
