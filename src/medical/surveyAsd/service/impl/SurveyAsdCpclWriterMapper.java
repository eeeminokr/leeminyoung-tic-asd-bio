...
public class SurveyAsdCpclWriterMapper implements CellWriterMapper<SurveyAsdCpclVO>{
	
	@Override
	public void mapHeader(Row row, SurveyAsdCpclVO vo) {
		Cell cell = null;
		int i = 0;
		...
		if(rowNum==0) {
			...
			cell.setCellValue("문장");
			
			cell = row.createCell(8);
			cell.setCellValue("장애진단");
			
			cell = row.createCell(9);
			cell.setCellValue("담화관리");
			
			cell = row.createCell(19);
			cell.setCellValue("상황에 따른 조절");
			
			cell = row.createCell(33);
			cell.setCellValue("의사소통 기능");
	...
	@Override
	public void mapRow(Row row, SurveyAsdCpclVO vo) {
		Cell cell = null;
		int i = 0;
		cell = row.createCell(i++);
		...
		cell = row.createCell(i++);
		cell.setCellValue(vo.getCpclExecDate());
		
		cell = row.createCell(i++);
		cell.setCellValue(vo.getSentence());
		
		cell = row.createCell(i++);
		cell.setCellValue(vo.gettDiagnosis());
	...
	public void cellMerge(Sheet sheet) {
		...
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 19, 32));
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 33, 45));
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 46, 52));
	}
