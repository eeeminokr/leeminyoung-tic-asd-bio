...
function gridChg(title,pageNum,rNum){
	...
	}else if(title == "환경물질분석"){ 
		analysisTitle = "환경물질분석";
		chgColumn = columnEnvMatt;
		searchParams.docNm = '환경물질분석';
	}else if(title == "유전자분석"){
		analysisTitle = "유전자분석";
		chgColumn = columnGen;
		searchParams.docNm = '유전자분석';
	}else if(title == "SNP분석"){
		analysisTitle = "SNP분석";
		chgColumn = columnSnp;
		searchParams.docNm = 'SNP분석';
	}
...
function uploadExcelDownload() {
	...
	location.href = GLOBAL_CONTEXT_PATH + '/downFile.view?dir=file.sample.dir&fileName=analysisUpload.xlsx';
}
